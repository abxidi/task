...
struct AppSidebar: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            sectionLabel("工作台")
            ForEach(AppRoute.workbenchNavigation) { route in
                navRow(
                    route,
                    title: route.sidebarTitle,
                    symbol: route.sidebarSymbol,
                    count: navigationCount(for: route)
                )
            }
            ...
            VStack(alignment: .leading, spacing: 8) {
                navRow(.settings, title: AppRoute.settings.sidebarTitle, symbol: AppRoute.settings.sidebarSymbol)
    ...
    private func navigationCount(for route: AppRoute) -> Int? {
        switch route {
        case .priorityMap, .taskList:
            openCount
        case .focusPool:
            focusEntries.count
        case .projectBoard, .insights, .settings:
            nil
        }
    }
}
